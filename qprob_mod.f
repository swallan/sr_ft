...
C   QLGAMA IS USER-WRITTEN FUNCTION.
      F2 = DF * 0.5Q0
      F2LF = F2 * LOG (DF) - DF * R2 - QLGAMA (F2)
      F21 = F2 - 1.0
      FF4 = DF * 0.25Q0
C
...
C           IF EXP(T1) < 9E-14, THEN DOESN'T CONTRIBUTE TO INTEGRAL
     ...
     #        THEN
                IF (IHALFQ .LT. JJ)
     #          THEN
                  QSQZ = Q * SQRT ((XLEGQ(J) * ULEN + TWA1) * 0.5Q0)
                ELSE
                  QSQZ = Q * SQRT ((-XLEGQ(J) * ULEN + TWA1) * 0.5Q0)
                ENDIF
C
...
C   FIND (F(W/2) - 1) ** CC (FIRST TERM IN INTEGRAL OF HARTLEY'S FORM).
      WPROB = ERF (QSQZ / QSQR2)
C   IF WPROB ** CC < 2E-22 THEN SET WPROB = 0
      IF (WPROB .GE. EXP(EPS2 / CC))
     #THEN
        WPROB = WPROB ** CC
      ELSE
        WPROB = 0.0
...
C           IF RINSUM ** (CC-1) < 9E-14, THEN DOESN'T CONTRIBUTE TO INTEGRAL
              IF (RINSUM .GE. EXP (EPS1 / CC1))
     #        THEN
                RINSUM = ALEG(J) * EXP (-0.5Q0 * QEXPO)
     #            * RINSUM ** CC1
                ELSUM = ELSUM + RINSUM
              ENDIF
C
...
C   THE ERROR FOR X = 50 IS 2.5E-24.
     ...
     #    * (1.0 - Y / O13 * (1.0 - Y / O71)))) /  X
        QLGAMA = QLGAMA + GSQR2P + (X - 0.5Q0) * LOG(X) - X
      ENDIF
C
  900 RETURN
      END
